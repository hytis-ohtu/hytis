...
test.describe("PersonSearch", () => {
  ...
  test("search is case-insensitive", async ({ page }) => {
    // Test lowercase
    let searchInput = page.getByPlaceholder("Hae henkilöä nimellä...");
    await searchInput.fill("matti");
    ...
    await searchInput.fill("");
    searchInput = page.getByPlaceholder("Hae henkilöä nimellä...");
    await searchInput.fill("MaTtI");
    await expect(page.getByText("Matti Virtanen")).toBeVisible();
  });
  ...
  test("search results count is displayed", async ({ page }) => {
    ...
    const resultText = await page
      .getByTestId("person-search-results-count")
      .textContent();
